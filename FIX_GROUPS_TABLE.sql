-- Fix Groups System
-- This script ensures the groups and group_members tables exist with all required fields
...
CREATE TABLE IF NOT EXISTS groups (
  ...
  key VARCHAR(255) UNIQUE NOT NULL DEFAULT gen_random_uuid()::text,
...
-- Add key column if it's missing
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ADD COLUMN key VARCHAR(255) UNIQUE NOT NULL DEFAULT gen_random_uuid()::text;
  END IF;
END $$;
...
-- ============================================
-- 6. DISABLE RLS FOR SERVICE ROLE
-- ============================================

-- We're using service role which bypasses RLS, but let's ensure RLS is properly configured
...
GRANT ALL ON group_invitations TO service_role;
GRANT ALL ON group_resource_access TO service_role;
GRANT ALL ON group_activity_log TO service_role;
...
BEGIN
  ...
  RAISE NOTICE 'Verified tables: groups (with key field), group_members, group_invitations, group_resource_access, group_activity_log';
END $$;
