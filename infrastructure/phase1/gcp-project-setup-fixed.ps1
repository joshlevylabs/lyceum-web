# Lyceum GCP Project Setup - PowerShell Version (Fixed)
...
gcloud projects create $ProjectId --name="$ProjectName" 2>$null
if ($LASTEXITCODE -eq 0) {
...
gcloud iam service-accounts create $saName --display-name="Lyceum Cluster Manager" --description="Service account for managing Lyceum optimized clusters" 2>$null
if ($LASTEXITCODE -eq 0) {
...
gcloud iam service-accounts keys create "$keysDir/lyceum-cluster-manager.json" --iam-account=$saEmail
if ($LASTEXITCODE -eq 0) {
