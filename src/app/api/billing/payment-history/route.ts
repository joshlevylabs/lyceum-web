...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch main app subscriptions with payments
    const { data: mainSubscriptions, error: mainError } = await supabase
      .from('subscriptions')
      .select('*')
      .eq('user_id', userId)
      .in('subscription_type', ['trial', 'paid'])
    ...
    // Add main app paid subscriptions
      ...
      mainSubscriptions.forEach(sub => {
        if (sub.subscription_type === 'paid' && sub.amount_paid_cents > 0) {
          paymentHistory.push({
            id: sub.id,
            date: sub.created_at,
            description: 'Lyceum Native App - Lifetime License',
      ...
      pluginSubscriptions.forEach(sub => {
        if (sub.subscription_type === 'paid' && sub.amount_paid_cents > 0) {
          ...
          paymentHistory.push({
            id: sub.id,
            date: sub.created_at,
            description: `${pluginName} - Lifetime License`,
            amount_cents: sub.amount_paid_cents,
            currency: sub.currency || 'usd',
