-- =============================================
-- Test Data Integration Migration (FIXED v2)
-- =============================================
-- This version handles cluster_projects without owner_id
-- Ownership is tracked through unified_clusters.created_by
...
CREATE TABLE IF NOT EXISTS public.test_data_measurements (
    ...
    -- Owner tracked via cluster, not directly
    -- We'll use a computed column or join to get owner
...
-- This version gets owner from unified_clusters.created_by
CREATE OR REPLACE VIEW public.test_data_projects_summary AS
SELECT
    cp.id,
    cp.cluster_id,
    uc.created_by AS owner_id,  -- Get owner from unified_clusters
    cp.name,
    cp.description,
    cp.project_type,
    cp.metadata,
...
GROUP BY cp.id, uc.id, uc.created_by;
...
BEGIN
    ...
    LEFT JOIN public.test_data_measurements tdm ON cp.id = tdm.cluster_project_id
    LEFT JOIN public.test_data_files tdf ON cp.id = tdf.cluster_project_id
    WHERE uc.created_by = p_user_id
    AND cp.project_type = 'test_data';
END;
$$ LANGUAGE plpgsql;

-- Function: Get owner of a cluster project (helper for RLS)
CREATE OR REPLACE FUNCTION get_cluster_project_owner(
    p_cluster_project_id UUID
)
RETURNS UUID AS $$
DECLARE
    v_owner_id UUID;
BEGIN
    SELECT uc.created_by INTO v_owner_id
    FROM public.cluster_projects cp
    JOIN public.unified_clusters uc ON cp.cluster_id = uc.id
    WHERE cp.id = p_cluster_project_id;
...
-- Test data measurements policies (owner determined through cluster)
CREATE POLICY "Users can view their own measurements"
    ON public.test_data_measurements FOR SELECT
    USING (
        auth.uid() = get_cluster_project_owner(cluster_project_id)
    );
...
-- Test data files policies (owner determined through cluster)
