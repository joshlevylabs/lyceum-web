...
-- This script creates the database schema for managing onboarding sessions
-- tied to trial licenses with plugin-specific requirements
...
CREATE TABLE IF NOT EXISTS public.onboarding_requirements (
  ...
  required_sessions INTEGER NOT NULL DEFAULT 3, -- Base 3 for centcom, +1 for each plugin
...
CREATE TABLE IF NOT EXISTS public.onboarding_sessions (
  ...
  session_number INTEGER NOT NULL DEFAULT 1, -- 1, 2, 3, etc.
  title TEXT NOT NULL,
  description TEXT,
  ...
  assigned_admin_id UUID REFERENCES public.user_profiles(id), -- Admin conducting the session
...
CREATE POLICY IF NOT EXISTS "Everyone can view onboarding requirements" ON public.onboarding_requirements
  FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Admin users can manage onboarding requirements" ON public.onboarding_requirements
  FOR ALL USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "Users can view their own onboarding sessions" ON public.onboarding_sessions
  FOR SELECT USING (
    auth.uid() = user_id OR 
...
CREATE POLICY IF NOT EXISTS "Admin users can manage onboarding sessions" ON public.onboarding_sessions
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.user_profiles up 
      WHERE up.id = auth.uid() 
...
CREATE POLICY IF NOT EXISTS "Users can view their own onboarding progress" ON public.onboarding_progress
  FOR SELECT USING (
    auth.uid() = user_id OR
    EXISTS (
      SELECT 1 FROM public.user_profiles up 
...
CREATE POLICY IF NOT EXISTS "Admin users can manage onboarding progress" ON public.onboarding_progress
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.user_profiles up 
      WHERE up.id = auth.uid() 
...
CREATE POLICY IF NOT EXISTS "Users can view their own onboarding reminders" ON public.onboarding_reminders
  FOR SELECT USING (
    auth.uid() = user_id OR
    EXISTS (
      SELECT 1 FROM public.user_profiles up 
...
CREATE POLICY IF NOT EXISTS "Admin users can manage onboarding reminders" ON public.onboarding_reminders
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.user_profiles up 
      WHERE up.id = auth.uid() 
