-- ====================================
-- LYCEUM ONBOARDING SYSTEM SETUP (FIXED)
...
-- First, create the license_keys table if it doesn't exist
CREATE TABLE IF NOT EXISTS public.license_keys (
  ...
  time_limit_type TEXT DEFAULT 'trial_30' CHECK (time_limit_type IN ('trial_30', 'trial_custom', 'unlimited')),
...
-- Create user_profiles table if it doesn't exist (needed for foreign keys)
...
-- Tracks individual onboarding sessions for users (FIXED: only reference license_keys)
...
-- Tracks overall onboarding progress for each user/license combination (FIXED: only reference license_keys)
...
-- First ensure your user profile exists
INSERT INTO public.user_profiles (id, email, username, full_name, company, role, created_at, updated_at, is_active) VALUES
(
  '2c3d4747-8d67-45af-90f5-b5e9058ec246', -- Your actual user ID from the terminal logs
  'josh@thelyceum.io',
  'josh',
  'Josh Admin',
  'The Lyceum',
...
(
  ...
  5,
  10,
  '["basic_access", "trial_features"]'::jsonb,
  '["centcom", "analytics"]'::jsonb,
  NOW() + INTERVAL '30 days',
  '2c3d4747-8d67-45af-90f5-b5e9058ec246', -- Your user ID
  NOW(),
  NOW()
) ON CONFLICT (id) DO UPDATE SET
...
DO $$
BEGIN
  RAISE NOTICE 'ONBOARDING SYSTEM SETUP COMPLETE!';
  RAISE NOTICE '✅ Tables created: onboarding_requirements, onboarding_sessions, onboarding_progress, onboarding_reminders';
  RAISE NOTICE '✅ License tables ensured: license_keys, user_profiles';
  RAISE NOTICE '✅ Sample data added for josh@thelyceum.io';
  RAISE NOTICE '✅ Admin permissions granted';
  RAISE NOTICE '';
  RAISE NOTICE '🎯 You can now access: http://localhost:3594/admin/onboarding';
END $$;
